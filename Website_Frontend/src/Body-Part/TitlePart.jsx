...
export default function TitlePart() {
  ...
  const addToCart = (imageSrc) => {
    console.log(cart.includes(imageSrc));
    if (!cart.includes(imageSrc)) {
      setCart([...cart, imageSrc]);
    }
  };

  const removeFromCart = (imageSrc) => {
    setCart(cart.filter((item) => item !== imageSrc));
  };
                  ...
                  <div className="card-body">
                    ...
                    {!cart.includes(item.imageUrl) ? (
                      <button
                        ...
                        onClick={() => addToCart(item.imageUrl)}
                        style={{
                          transition:
